...
```
```{r fig-paired-CR2, echo=FALSE, fig.align='center', out.width='48%', fig.pos='H', fig.cap="Histogram of Difference between daily CR"}
knitr::include_graphics(c(
  here::here("results_paired_dailyCR", "02_diff_hist_with_CI_dailyCR.png")
))
```
...
```
###**Conclusion:**  
- The histogram of paired differences shows that most daily CR differences are clustered near zero with a slight left shift, indicating that the Test group tended to have marginally lower CRs than the Control group. The 95% confidence interval of the mean difference (−5.52%, 0.76%) crosses zero, suggesting that this small negative difference is not statistically significant. The distribution appears approximately symmetric, supporting the normality assumption of the paired t-test.
